...
public class EmpregadoRest {
	...
	@Path("/ativos")
	@Produces(MediaType.APPLICATION_JSON)
	public Response listarEmpregadosAtivos() {
		try {
			...
			return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
					.entity("Erro ao recuperar empregados ativos: " + e.getMessage()).build();
		}
	}
	...
	@Path("/inativos")
	@Produces(MediaType.APPLICATION_JSON)
	public Response listarEmpregadosInativos() {
		try {
			...
			return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
					.entity("Erro ao recuperar empregados inativos: " + e.getMessage()).build();
		}
	}
	...
	public Response excluirEmpregado(@PathParam("matricula") String matricula) throws Exception {
		try {
			if (Banco.buscarEmpregadoPorMatricula(matricula) == null) {
				return Response.status(Status.NOT_FOUND).entity("{ \"mensagem\" : \"Empregado não encontrado!\" }")
						.build();
			}
			Banco.excluirEmpregado(matricula);
			return Response.ok().entity("{ \"mensagem\" : \"Empregado " + matricula + " excluído!\" }").build();
		} catch (Exception e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR)
					.entity("{ \"mensagem\" : \"Falha na exclusão do empregado!\" , \"detalhe\" :  \"" + e.getMessage()
							+ "\"  }")
					.build();
		}
	}
